...
#####################################################################
    ...
    def build_finetuning_model(self):
        ...
        # compile the model
        finetune_model.compile(loss="categorical_crossentropy", optimizer=optimizers.SGD(lr=0.01, momentum=0.9),
                            metrics=["accuracy"])

        #prepare train, test, validation
        x_train = None
        y_train = None
    ...
    def train_and_test_model_dividePart(self,num_of_pair_ratio=1):
        indel_list = []#read from file/ datapath# modify here! need finish
        ...
        #shuffle(indel_list)
        #shuffle(indel_list)
        #batch_num_each_divide = int(round(len(indel_list)/divide_part))
        #batch_num_each_divide = 1
        #########for divide TF test################
        divide_batch_list = [0,4,9,13]
        for i in range(0, divide_part):
            #index_from = i * (batch_num_each_divide)
            #index_to = (i+1)*batch_num_each_divide-1
            index_from = divide_batch_list[i]
            index_to = divide_batch_list[i+1]
            test_indel = indel_list[index_from:index_to]
            #for test_indel in range (self.num_batches):                                      #  for GTRD leave-one-TF-out CV
            self.update_test_train_data(test_indel,self.epochs,num_of_pair_ratio)
            self.construct_model(self.x_train)
            model = self.model
...
def main():
    ...
    tcs = direct_model1_squarematrix(num_batches=13,
        data_path="/home/comp/jxchen/bone_Marrow_topcorr/boneMarrow_TFdivideNew_topcorr20/version11/",
        output_dir="out_co-train3_topcorr20/")
    tcs.train_and_test_model_dividePart_assignTForder(num_of_pair_ratio=1)


def main_predict():
    #weight_path = "boneMarrow_co-train3_trained_model_shallow.h5"
    #weight_path = "/Users/jiaxchen2/Desktop/realdata/to_predict/boneMarrow_training_keras_cnn_trained_model_shallow.h5"
    weight_path = "/Users/jiaxchen2/Desktop/realdata/to_predict/mesc_training_weights.hdf5"
    #data_path = "/Users/jiaxchen2/Desktop/realdata/dendritic/denritic_TFdivide/version11/"
    data_path = "/Users/jiaxchen2/Desktop/realdata/to_predict/boneMarrow_TFdivideNew_topcorr10/version11/"
    #data_path = "/Users/jiaxchen2/Desktop/realdata/to_predict/mesc_TFdivideNew/version11/"

    predict_output_dir = "/Users/jiaxchen2/Desktop/realdata/to_predict/predict_boneMarrow_withmescWeight/"
    #predict_output_dir = "/Users/jiaxchen2/Desktop/realdata/to_predict/predict_mesc_withboneMarrowWeight/"
    num_batches = 13
    #num_batches = 38
