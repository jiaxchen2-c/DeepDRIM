...
class ChipSeq_data_convert:
    ...
    def work_filter_positive_pair(self):
        self.flag_filter=True
        self.filter_by_corr=True
        self.corr_cutoff=0.01
        if self.flag_filter:
            #read expr table, 2 matrix and merge
            [expr1,geneNames1] = self.read_sparse_expr_table("/Users/jiaxchen2/Desktop/realdata/chip-seq/data_hcc/GSE140228_UMI_counts_Droplet.mtx",
                                        "/Users/jiaxchen2/Desktop/realdata/chip-seq/data_hcc/GSE140228_UMI_counts_Droplet_genes.tsv",1,
                                        "/Users/jiaxchen2/Desktop/realdata/chip-seq/data_hcc/GSE140228_UMI_counts_Droplet_cellinfo.tsv",0)
            [expr2,geneNames2] = self.read_expr_table("/Users/jiaxchen2/Desktop/realdata/chip-seq/data_liver/gse124395_normalhumanlivercellatlasdata.txt")
            #get all genes in this table
            [self.expr,self.geneNames]=self.concat_matrix(expr1,geneNames1,expr2,geneNames2)
            #sparse_matrix = scipy.sparse.load_npz('/Users/jiaxchen2/Desktop/realdata/chip-seq/expr_sparse_matrix.npz')

            self.save_rpkm_and_geneNames('/Users/jiaxchen2/Desktop/realdata/chip-seq/expr_sparse_matrix.npz',
                                         "/Users/jiaxchen2/Desktop/realdata/chip-seq/geneNames.txt")
            # geneName to geneName map for representation generation
            self.output_geneName_to_geneName_map("/Users/jiaxchen2/Desktop/realdata/chip-seq/geneName_map.txt")
        ...
        self.read_positive_pairs_and_filter("/Users/jiaxchen2/Desktop/realdata/chip-seq/positive_pairs.csv")
        # output filtered positive pairs
        self.output_positive_pairs_according_to_adj_matrix("/Users/jiaxchen2/Desktop/realdata/chip-seq/filtered_positive_pairs.txt")
        #output training pairs
        self.output_training_pairs_according_to_adj_matrix("/Users/jiaxchen2/Desktop/realdata/chip-seq/training_pairs.txt")

    def load_initial_expr(self):
        # read expr table, 2 matrix and merge
        [expr1, geneNames1] = self.read_sparse_expr_table("data_hcc/GSE140228_UMI_counts_Droplet.mtx",
                                                          "data_hcc/GSE140228_UMI_counts_Droplet_genes.tsv", 1,
                                                          "data_hcc/GSE140228_UMI_counts_Droplet_cellinfo.tsv", 0)
        [expr2, geneNames2] = self.read_expr_table("data_liver/gse124395_normalhumanlivercellatlasdata.txt")
        # get all genes in this table
        [self.expr, self.geneNames] = self.concat_matrix(expr1, geneNames1, expr2, geneNames2)
        for i in range(0,len(self.geneNames)):
            self.geneNames[i] = self.geneNames[i].lower()
        ...
        self.save_rpkm_and_geneNames('expr_sparse_matrix.npz',
                                     "geneNames.txt")
        # geneName to geneName map for representation generation
        self.output_geneName_to_geneName_map("geneName_map.txt")
        self.concat_cell_info()
    ...
    def concat_cell_info(self):
        filename1="/Users/jiaxchen2/Desktop/realdata/chip-seq/data_hcc/GSE140228_UMI_counts_Droplet_cellinfo.tsv"
        index1=0
        filename2 = "/Users/jiaxchen2/Desktop/realdata/chip-seq/data_liver/gse124395_clusterpartition.txt"
        index2 = 0

        X1=pd.read_table(filename1,header='infer')
        cell_id1=X1.iloc[:,index1]
        X2 = pd.read_table(filename2, header='infer')
        cell_id2 = X2.iloc[:, index2]
        print('cell_id2',cell_id2)

        cell_id=np.concatenate((cell_id1,cell_id2))
        print('cell_id',cell_id)
        study1=np.full((len(cell_id1), ), 'hcc')
        print('study1',study1)
        study2 = np.full((len(cell_id2),), 'healthy_liver')
        study = np.concatenate((study1,study2))
        print('study',study)
        np.savetxt('/Users/jiaxchen2/Desktop/realdata/chip-seq/combined_study_info.txt',study,fmt="%s")
        np.savetxt('/Users/jiaxchen2/Desktop/realdata/chip-seq/combined_cell_id.txt',cell_id,fmt="%s")
    ...
    def load_lung_expr(self, expr_file,ID_to_name_mapfile,id_col,name_col):
        ...
        self.geneNames_tmp = df.index
        self.geneNames_tmp = np.asarray(self.geneNames_tmp)
        self.geneNames_tmp = self.geneNames_tmp.ravel()

        print("geneNames",self.geneNames_tmp)

        self.geneNames = []
        self.geneIDs = []
        keep_rows=[]
        for i in range(0, len(self.geneNames_tmp)):
            namei=map_dict.get(self.geneNames_tmp[i])
            if namei is not None:
                print("get genename:",self.geneNames_tmp[i], namei)
                self.geneNames.append(namei)
                self.geneIDs.append(self.geneNames_tmp[i])
                keep_rows.append(i)
            else:
                print("can not find self.geneNames_tmp[i]", self.geneNames_tmp[i])
        ...
        self.geneIDs = np.asarray(self.geneIDs)
        self.geneIDs = self.geneIDs.ravel()
        self.expr=self.expr[keep_rows,:]
        print("keep expr shape", self.expr.shape)
    ...
    def work_filter_positive_pair_server_healthy_liver(self, select_str='healthy_liver'):
        self.filter_by_corr = True
        self.flag_remove_corr_NA = False
        self.flag_filter = True
        self.corr_cutoff = 0.01
        self.flag_negative_also_large_corr = True
        if self.flag_filter:
            # self.load_initial_expr()
            self.load_combined()
            self.filter_cell_from_cell_info_file('combined_study_info.txt',select_str)
            ...
            corr_list = self.corr_matrix.ravel()
            corr_list_remove_infinite = corr_list[np.isfinite(corr_list)]
            hist_corr, bin_edges_corr = np.histogram(corr_list_remove_infinite, bins=40)
            np.savetxt(select_str+"hist_corr_remove_infinite.txt", hist_corr)
            np.savetxt(select_str+"bin_edges_corr_remove_infinite.txt", bin_edges_corr)

            self.geneIDs = self.get_geneIDs_from_geneName_for_GTEx_positive(self.geneNames,
                                                                            ID_to_name_mapfile="genes_ID_symbol_map.csv",
                                                                            id_col=1, name_col=0, lower_flag=True)
            print("self.geneNames",self.geneNames)
            print("self.geneIDs",self.geneIDs)
            self.geneIDs = np.asarray(self.geneIDs)
            self.geneIDs = self.geneIDs.ravel()
            ...
            self.geneIDs = self.get_geneIDs_from_geneName_for_GTEx_positive(self.geneNames, ID_to_name_mapfile="genes_ID_symbol_map.csv",id_col=1,name_col=0)
            self.geneIDs = np.asarray(self.geneIDs)
            self.geneIDs = self.geneIDs.ravel()

        # read positive pair
        # filter positive pairs
        #self.read_positive_pairs_and_filter("liver_macs_positive_pairs__pvalue_e10_8.csv")



        self.read_positive_pairs_and_filter("Liver_GTEx_filtered_weight1_edges.csv",col0_get_by_name=True, col1_get_by_name=False)
        # output filtered positive pairs
        self.output_positive_pairs_according_to_adj_matrix("positive_pairs_GTEx_negLarger_corr0.01_"+select_str+".txt")
        # output training pairs
        self.output_training_pairs_according_to_adj_matrix("training_pairs_GTEx_negLarger_corr0.01_"+select_str+".txt")
    ...
    def work_filter_positive_pair_server_lung(self):
        self.filter_by_corr=True
        self.flag_remove_corr_NA=False
        self.flag_filter=True
        ...
        self.flag_negative_also_large_corr = True
        if self.flag_filter:
            self.load_lung_expr("lung_Allsamples_expr.txt", ID_to_name_mapfile="genes_ID_symbol_map.csv",id_col=1,name_col=0)
            #self.output_geneName_to_geneName_map("lung_geneName_map.txt")
        ...
        self.read_positive_pairs_and_filter("Lung_filtered_weight1.5_edges.csv",col0_get_by_name=True, col1_get_by_name=False)

        # output filtered positive pairs
        self.output_positive_pairs_according_to_adj_matrix("positive_pairs_GTEx_lung_negLarger_corr0.1.txt")
        self.output_geneName_to_geneName_map("lung_geneName_map_GTEx.txt")
        #output training pairs
        self.output_training_pairs_according_to_adj_matrix("training_pairs_GTEx_lung_negLarger_corr0.1.txt")
    ...
    def work_get_nonzeroTF_genes_liver(self,select_str='healthy_liver'): #need complete#'healthy_liver' or 'hcc'
        #output only candidate edges
        #3. by nmf corr cutoff??
        #generate list according to format like training pairs with labels as 3(to be predict)
        self.load_combined()
        ...
        filtered_TFs=self.geneNames[self.filtered_TF_rows]
        np.savetxt("nonzero_TF_symbol_healthy_liver.txt",filtered_TFs,"%s")
        filtered_genes = self.geneNames[self.filtered_candidate_genes_rows]
        np.savetxt("nonzero_genes_symbol_healthy_liver.txt", filtered_genes, "%s")
    ...
    def work_get_corr_for_train_pairs_liver(self, select_str='hcc',input_train_pair_file="training_pairs_nofilter_liver.txt",
                                            out_put_corr_file="training_pairs_nofilter_hcc_correlation.txt",flag_only_nonzero=True):
        self.load_combined()
        self.filter_cell_from_cell_info_file('combined_study_info.txt', select_str)
        self.calculate_corr_for_pairs(input_train_pair_file,out_put_corr_file,flag_only_nonzero)# check the filename

    def work_get_corr_for_train_pairs_lung(self, input_train_pair_file="training_pairs_nofilter_lung.txt",
                                            out_put_corr_file="training_pairs_lung_correlation.txt",flag_only_nonzero=True):
        self.load_lung_expr("lung_Allsamples_expr.txt", ID_to_name_mapfile="genes_ID_symbol_map.csv", id_col=1,
                            name_col=0)
        self.calculate_corr_for_pairs(input_train_pair_file,out_put_corr_file,flag_only_nonzero)# check the filename
...
def main():
    ...
    #tcs.work_chipSeq_to_positive_pair(tissue='lung')
    #tcs.work_chipSeq_to_positive_pair(tissue='liver')
    #tcs.work_chipSeq_to_positive_pair(tissue='brain',species="Mus_musculus")
    #tcs.work_chipSeq_to_positive_pair(tissue='embryonic cortex',species="Mus_musculus")
    #tcs.work_filter_positive_pair_server_combined()
    tcs.work_filter_positive_pair_server_healthy_liver('healthy_liver')
    #tcs.work_filter_positive_pair_server_healthy_liver('hcc')
    #tcs.work_filter_positive_pair_server_lung()
    #tcs.work_generate_gene_pairs_list_for_prediction('hcc',corr_cutoff=0.3)
    #tcs.work_get_nonzeroTF_genes_liver('healthy_liver')
    #tcs.work_get_corr_for_train_pairs_liver("hcc", "training_pairs_nofilter_liver.txt", "training_pairs_nofilter_hcc_nonZeroCorr.txt",True)
    #tcs.work_get_corr_for_train_pairs_liver("healthy_liver", "training_pairs_nofilter_liver.txt","training_pairs_nofilter_healthy_liver_nonZeroCorr.txt",True)

    #tcs.work_get_corr_for_train_pairs_liver("hcc", "training_pairs_nofilter_liver.txt","training_pairs_nofilter_hcc_normalCorr.txt", False)
    #tcs.work_get_corr_for_train_pairs_liver("healthy_liver", "training_pairs_nofilter_liver.txt","training_pairs_nofilter_healthy_liver_normalCorr.txt", False)

    #tcs.work_get_corr_for_train_pairs_lung("training_pairs_nofilter_lung.txt","training_pairs_nofilter_lung_nonZeroCorr.txt", True)
    #tcs.work_get_corr_for_train_pairs_lung("training_pairs_nofilter_lung.txt","training_pairs_nofilter_lung_normalCorr.txt", False)


def main_single_cell_type():
    tcs = ChipSeq_data_convert()
    #tcs.work_filter_positive_pair_single_cell_type(positive_pair_file="mESC-ChIP-seq-network.csv",expr_file="mESC/ExpressionData.csv",label="mESC")
    #tcs.work_filter_positive_pair_single_cell_type(positive_pair_file="HepG2-ChIP-seq-network.csv",expr_file="hHep/ExpressionData.csv",label="hHep")#???G2
    #tcs.work_filter_positive_pair_single_cell_type(positive_pair_file="hESC-ChIP-seq-network.csv",expr_file="hESC/ExpressionData.csv",label="hESC")
    #tcs.work_filter_positive_pair_single_cell_type(positive_pair_file="mDC-ChIP-seq-network.csv",expr_file="mDC/ExpressionData.csv",label="mDC")

    #tcs.work_filter_positive_pair_single_cell_type(positive_pair_file="mHSC-ChIP-seq-network.csv",expr_file="mHSC-E/ExpressionData.csv",label="mHSC_E")
    #tcs.work_filter_positive_pair_single_cell_type(positive_pair_file="mHSC-ChIP-seq-network.csv",expr_file="mHSC-L/ExpressionData.csv", label="mHSC_L")
    #tcs.work_filter_positive_pair_single_cell_type(positive_pair_file="mHSC-ChIP-seq-network.csv",expr_file="mHSC-GM/ExpressionData.csv", label="mHSC_GM")
    #tcs.work_filter_positive_pair_single_cell_type(positive_pair_file="mHSC-ChIP-seq-network.csv",expr_file="mHSC_combined/ExpressionData.csv", label="mHSC_combined")

    #for covid19


    #tcs.work_chipSeq_to_positive_pair(tissue='CD4')
    #tcs.work_filter_positive_pair_single_cell_type(positive_pair_file="CD4_macs_positive_pairs__pvalue_e10_8.csv", expr_file="health_CD4.csv", label="health_CD4")
    ...
    tcs.work_filter_positive_pair_single_cell_type(positive_pair_file="B_cell_macs_positive_pairs__pvalue_e10_8.csv",
                                                   expr_file="B_cell_proteincoding_expr_hcc.csv", label="B_cell_hcc")


def main_for_tissue():
    tcs = ChipSeq_data_convert()
    #tcs.work_chipSeq_to_positive_pair(tissue='embryonic cortex',species="Mus_musculus")
    #tcs.work_filter_positive_pair_single_cell_type(positive_pair_file="embryonic_cortex_macs_positive_pairs__pvalue_e10_8.csv",expr_file="GSE123335_E14_combined_matrix.txt",label="mouse_embryonic_cortex")
    #tcs.work_chipSeq_to_positive_pair(tissue='Drosophila_eye_disks', species="Drosophila_melanogaster")
    tcs.work_chipSeq_to_positive_pair(tissue='Drosophila_ovary', species="Drosophila_melanogaster")


if __name__ == '__main__':
    main_single_cell_type()
